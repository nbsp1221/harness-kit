---
spec_id: 20260430-0200-hk-spec-skill-contract
title: hk-spec Skill Contract
status: active
stage: spec
...
# hk-spec Skill Contract

## Problem

`harness-kit` has a repo-local starter contract, but it does not yet have the first workflow skill that starts a new spec item in a repeatable way.

Without `hk-spec`, humans and agents must manually create `specs/<YYYYMMDD-HHMM-short-slug>/`, copy lifecycle files, remember frontmatter fields, and decide how much of `spec.md` should be generated versus authored. That leaves the most important entry point of the workflow dependent on chat memory.
...
`hk-spec` should be a Codex-first Agent Skill installed by the starter template under the downstream repository's repo-local skill discovery path:
...
```text
.agents/skills/hk-spec/
```

This matches Codex's current repo-local skill discovery path.

Decision: Phase 1 starter includes `hk-spec` at `template/.agents/skills/hk-spec/`.
Without this, `hk-spec` would depend on global user state, an external CLI, plugin packaging, or docs-only manual behavior. Those alternatives weaken the repo-local starter goal because a target repository would no longer contain the workflow skill needed to start its own spec cycle.

Decision: `new-spec-item` lives inside `template/.agents/skills/hk-spec/scripts/`.
This script is the deterministic execution tool of the `hk-spec` skill itself, not a starter-wide command. Shared scripts remain reserved for starter-level commands such as `bootstrap`, `adopt`, and `doctor`, or for logic reused by multiple skills. If later `hk-plan`, `hk-verify`, or `hk-review` need the same implementation logic, it can be extracted after that duplication exists.

Decision: `hk-spec` is a project-bound `harness-kit` skill, not a standalone skill pack.
Lifecycle file templates remain under `template/specs/_templates/` as repo-level artifact templates. The `hk-spec` skill owns the workflow instructions and `new-spec-item` command that instantiate those templates, but it does not duplicate the lifecycle templates under skill `assets/`.
...
Decision: `new-spec-item --json` uses a stable lower-case harness-kit envelope rather than Spec Kit's minimal uppercase field set.
...
The description should trigger only when starting or creating a new harness-kit spec item from a feature idea, bug, research task, or workflow change.
...
Decision: tests for `hk-spec` get a dedicated file.
`tests/test_hk_spec_skill.py` owns skill layout, frontmatter, script behavior, dry-run JSON, lifecycle creation, repo-template use, conflict safety, and missing-template failures.
...
`doctor` owns whole-repository validation: starter files, lifecycle templates, existing spec items, existing frontmatter shape, and harness-kit config health.
...
- `R1`: `hk-spec` MUST be present in the starter template at `template/.agents/skills/hk-spec/`.
- `R2`: `template/.agents/skills/hk-spec/SKILL.md` MUST use Agent Skills-compatible frontmatter with `name: hk-spec` and a description that clearly triggers on creating or starting a new harness-kit spec item.
- `R3`: `hk-spec` MUST include a deterministic scaffolding script at `template/.agents/skills/hk-spec/scripts/new-spec-item`.
...
- `R15`: Phase 1 MUST install `hk-spec` into downstream repositories through the starter template, not through a global user skill path.
- `R16`: Phase 1 MUST NOT package `hk-spec` as a public plugin.
- `R17`: `hk-spec` MUST declare that it requires a `harness-kit` project structure with `specs/_templates/`.
...
- `R20`: `SKILL.md` MUST use a narrow description equivalent to: "Use when starting or creating a new harness-kit spec item from a feature idea, bug, research task, or workflow change. Scaffolds specs/<YYYYMMDD-HHMM-short-slug>/ with spec.md, plan.md, verification.md, and review.md, then authors the spec-stage problem, scope, requirements, success criteria, assumptions, and planning handoff. Do not use for implementation planning, code execution, verification, review, or editing an existing spec item."
- `R21`: The implementation MUST add dedicated `hk-spec` behavior tests in `tests/test_hk_spec_skill.py` and update starter inventory tests only for installation coverage.
- `R22`: `new-spec-item` MUST NOT perform whole-repository health validation; that responsibility remains with `doctor`.

## Success Criteria

- `SC1`: A user or agent can start a new spec item by invoking `hk-spec` without remembering the directory naming convention.
...
- starter-template `hk-spec` skill layout
...
- `hk-plan`, `hk-verify`, and `hk-review` implementation
- public plugin packaging
- global user installation
- Claude Code `.claude/skills/hk-spec/` adapter
- broad `--force`
...
- `A1`: `template/.agents/skills/hk-spec/` is the canonical Phase 1 repo-local installation source.
...
- `Q6`: Test layout is closed as dedicated `tests/test_hk_spec_skill.py` behavior tests plus minimal starter inventory updates in `tests/test_starter_scripts.py`.
...
- Local: `/tmp/harness-kit-research/oh-my-codex/skills/skill/SKILL.md`
- Local: `/tmp/harness-kit-research/oh-my-codex/skills/plan/SKILL.md`
- Local: `/tmp/harness-kit-research/compound-engineering-plugin/README.md`
- Local: `/tmp/harness-kit-research/compound-engineering-plugin/plugins/compound-engineering/skills/ce-plan/SKILL.md`
- Local: `/tmp/harness-kit-research/compound-engineering-plugin/plugins/compound-engineering/skills/ce-work/SKILL.md`
- Local: `/tmp/harness-kit-research/gstack/docs/skills.md`
- Local: `/tmp/harness-kit-research/gstack/scripts/discover-skills.ts`
- Local: `/tmp/harness-kit-research/gstack/test/skill-validation.test.ts`
...
- Local: `/tmp/harness-kit-extra/spec-kit/scripts/bash/create-new-feature.sh`
- Local: `/tmp/harness-kit-extra/spec-kit/tests/integrations/test_integration_base_skills.py`
- Local: `/tmp/harness-kit-extra/spec-kit/tests/integrations/test_integration_codex.py`
