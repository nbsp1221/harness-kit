...
Prepare `wayrail` for later npm publication as `@retn0/wayrail` without publishing it yet.
...
There should be no Python runtime dependency for core `wayrail` commands after this work is complete.

## Requirements Trace

| Spec ID | Requirement | Plan Coverage | Verification |
| --- | --- | --- | --- |
| R1 | Package name `@retn0/wayrail` | Unit 1 `package.json` | Package metadata test |
| R2 | Public scoped publishing config | Unit 1 `publishConfig.access` | Package metadata test |
| R3 | Global `wayrail` binary | Unit 1 bin map, Unit 2 Node CLI | Package metadata and CLI help tests |
| R4 | Fallback `wyr` alias | Unit 1 bin map, Unit 2 Node CLI | Package metadata and CLI help tests |
...
- Create npm-facing Node CLI, likely `bin/wayrail.js`.
...
- `wayrail` and `@retn0/wayrail` returned npm 404 on 2026-06-08.
- The package should use scoped package ownership while still providing `wayrail` and `wyr` binaries.

## Decisions

| Decision | Rationale | Alternatives Considered | Requirements Served |
| --- | --- | --- | --- |
| Use `@retn0/wayrail` as package name | Matches owner namespace and avoids unscoped/third-party brand collision | unscoped `wayrail`, `wayrail-starter` | R1, R2 |
...
| Provide both `wayrail` and `wyr` bins | Natural command plus short fallback for future binary conflicts | only `wayrail`, only `wyr` | R3, R4 |
...
- [ ] Unit 1: Add npm package metadata
  ...
  - Approach:
    ...
    - Set `bin` to map `wayrail` and `wyr` to `./bin/wayrail.js`.
...
- [ ] Unit 2: Add native npm CLI
  ...
  - Verification:
    ...
    - CLI can run command help, e.g. `node bin/wayrail.js bootstrap --help`.
    - CLI behavior tests cover bootstrap/adopt/doctor.

- [ ] Unit 3: Remove replaced Python CLI implementation
  - Requirements: R15, R18, SC5
  - Files:
    - Delete `scripts/wayrail/bootstrap`
    - Delete `scripts/wayrail/adopt`
    - Delete `scripts/wayrail/doctor`
    - Delete `scripts/wayrail/_lib/starter.py`
    - Remove empty `scripts/wayrail/_lib/` if applicable
  - Depends on: Unit 2
  - Approach:
    - Remove the Python implementation after Node tests cover the same behavior.
    - Update references in tests and docs from `scripts/wayrail/` to npm CLI paths.
    - Do not keep Python as a reference/test oracle.
  - Verification:
    - Tests assert the removed Python CLI files no longer exist.
    - `npm pack --dry-run --json` does not include `scripts/wayrail/`.
...
- [ ] Unit 4: Add npm usage documentation
  ...
  - Depends on: Unit 1
  - Approach:
    - Document package name: `@retn0/wayrail`.
    - Document intended future install command: `npm install -g @retn0/wayrail`.
...
- [ ] Unit 5: Add packaging and behavior tests
  ...
  - Approach:
    ...
    - Run `node bin/wayrail.js --help` and `node bin/wayrail.js bootstrap --help`.
    ...
    - Run `npm pack --dry-run --json` and assert expected files are included:
      ...
      - `README.md`
      - `bin/wayrail.js`
      - key `template/` files and `wr-*` skill files
...
- package metadata has `name: @retn0/wayrail`
- `bin.wayrail` and `bin.wyr` both point to the Node CLI
...
- `wayrail` binary may conflict with another globally installed package.
  - Mitigation: also expose `wyr`.
