...
`wayrail` currently has repository scripts under `scripts/wayrail/` and a starter template under `template/`, but it is not yet shaped as an npm-installable CLI package.
...
This blocks the next product step: making `wayrail` easy to install and use as a personal, opinionated repo-local agent workflow starter.
...
- `wr-spec`, `wr-plan`, `wr-verify`, and `wr-review` are installed through the starter template.
...
The unscoped `wayrail` package and scoped `@retn0/wayrail` package returned npm 404 on 2026-06-08.
Because this project is primarily `retn0`'s opinionated starter that others may use if they want, the package should use the scoped name `@retn0/wayrail`.

## Requirements

- `R1`: The npm package name MUST be `@retn0/wayrail`.
- `R2`: The package MUST be configured for public scoped npm publishing.
- `R3`: The package MUST expose a global `wayrail` binary.
- `R4`: The package SHOULD expose a shorter fallback binary alias, `wyr`, to avoid global command conflicts.
...
- `R9`: The package metadata MUST include a clear description that this is `retn0`'s opinionated wayrail starter for repo-local agent workflows.
...
- `SC1`: A maintainer can run local package checks and see that `@retn0/wayrail` would package the expected files.
- `SC2`: A maintainer can install or execute the local package entrypoint and run `wayrail bootstrap`, `wayrail adopt`, and `wayrail doctor` behavior through the npm-facing command path.
...
- `SC5`: The repository no longer contains the existing Python CLI implementation under `scripts/wayrail/`.
...
- The work should account for possible future global binary conflicts with the `wayrail` binary.
- The package should be public even though it is opinionated for personal use.

## Assumptions

- `A1`: The npm account or organization has permission to publish `@retn0/wayrail`.
- `A2`: `@retn0/wayrail` is currently unclaimed on npm based on `npm view @retn0/wayrail` returning 404 on 2026-06-08.
- `A3`: Users who install this package are expected to have Node available.
- `A4`: Users who install this package should not need Python for core `wayrail` commands.
...
- `A6`: The fallback `wyr` binary is useful as a short command and as a hedge against future binary conflicts.
...
- Local: `scripts/wayrail/bootstrap`
- Local: `scripts/wayrail/adopt`
- Local: `scripts/wayrail/doctor`
- Local: `scripts/wayrail/_lib/starter.py`
- Local: `template/`
- npm registry: `npm view wayrail` returned 404 on 2026-06-08
- npm registry: `npm view @retn0/wayrail` returned 404 on 2026-06-08
