# wr-spec Skill Contract Plan

## Overview

Implement the first repo-local wayrail workflow skill: `wr-spec`.
The starter template will install `.agents/skills/wr-spec/` into downstream repositories.
...
- Add `template/.agents/skills/wr-spec/SKILL.md`.
- Add `template/.agents/skills/wr-spec/agents/openai.yaml`.
- Add executable `template/.agents/skills/wr-spec/scripts/new-spec-item`.
- Ensure starter inventory installs/adopts the new skill files.
- Add `tests/test_wr_spec_skill.py`.
- Extend `tests/test_starter_scripts.py` only for template inventory coverage.

Not in this plan:

- `wr-plan`, `wr-verify`, or `wr-review`.
...
- OpenAI Codex and Agent Skills guidance put trigger responsibility on `name` and `description`, so `wr-spec` must have a narrow description and body-level compatibility guard.
- Existing wayrail starter scripts already use lower-case structured JSON with `dry_run`, `safe_to_apply`, `actions`, and explicit conflicts.
- `doctor` patterns in wayrail, Spec Kit, npm, Flutter, and Homebrew treat health checks as diagnostic commands, separate from create/scaffold commands.
...
- when to use
- when not to use
- compatibility: requires wayrail starter structure and `specs/_templates/`
- workflow: run `scripts/new-spec-item` first, then author generated `spec.md`
- spec authoring rules
- completion: report created spec path and recommend `wr-plan`
...
Add `tests/test_wr_spec_skill.py` with coverage for:

- template skill files exist
- frontmatter has `name: wr-spec` and narrow description
...
```bash
pytest tests/test_wr_spec_skill.py tests/test_starter_scripts.py
```
