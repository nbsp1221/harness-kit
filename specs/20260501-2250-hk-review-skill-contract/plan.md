# hk-review Skill Contract Plan

## Overview

Implement `hk-review` as the fourth repo-local harness-kit lifecycle skill.
...
This plan keeps Phase 1 intentionally light: `hk-review` is a `SKILL.md` workflow plus minimal agent metadata, not a deterministic review runner, PR tool, waiver engine, or code-fixing tool.

## Requirements Trace

| Spec ID | Requirement | Plan Coverage | Verification |
| --- | --- | --- | --- |
| R1 | Starter contains `template/.agents/skills/hk-review/` | Unit 1 creates skill directory | File existence tests |
...
- starter inventory updates so bootstrap/adopt install `hk-review`
- dedicated `hk-review` skill contract tests
- updating this spec item's verification/review artifacts later in the lifecycle

Out of scope for this pass:

- `template/.agents/skills/hk-review/scripts/`
...
- `specs/20260501-2250-hk-review-skill-contract/spec.md`
...
- Skill-specific contract tests live in dedicated `tests/test_hk_<stage>_skill.py` files.
- Current tests use Python `unittest`, with `uvx pytest` as the normal runner.

Local design context:

- `hk-verify` now writes `verification.md` with Review Handoff expectations.
- `hk-review` should consume that handoff, challenge evidence quality, and produce a final review artifact.
...
| --- | --- | --- | --- |
| Implement `hk-review` as skill prose plus metadata only | Review requires judgment, evidence synthesis, and prioritization | Add a review runner script now | R5-R49 |
...
| Keep risk acceptance human-owned | Prevents `hk-review` from becoming a waiver engine | Let review mark accepted risks | R26-R32, R41 |
...
- [ ] Unit 1: Add `hk-review` skill source
  ...
  - Files:
    - Create `template/.agents/skills/hk-review/SKILL.md`
    - Create `template/.agents/skills/hk-review/agents/openai.yaml`
  - Depends on: none
  - Approach:
    - Mirror the concise structure of `hk-verify`.
    - Put `name: hk-review` and a narrow description in frontmatter.
...
- [ ] Unit 2: Add `hk-review` to starter inventory
  - Requirements: R1, R50
  - Files:
  ...
  - Depends on: Unit 1
  - Approach:
    - Add `.agents/skills/hk-review/SKILL.md` and `.agents/skills/hk-review/agents/openai.yaml` to both required template file lists.
    - Do not add executable-bit assertions because `hk-review` has no script in Phase 1.
  - Verification:
    - Starter template file test passes.
    - Bootstrap fixture creates the `hk-review` files.

- [ ] Unit 3: Add dedicated `hk-review` contract tests
  - Requirements: R2-R51
  - Files:
    - Create `tests/test_hk_review_skill.py`
  - Depends on: Unit 1
  - Approach:
    - Assert skill files exist.
    - Assert frontmatter has `name: hk-review`.
    - Assert trigger text mentions reviewing an implemented and verified harness-kit spec item into `review.md`.
  ...
  - Verification:
    - `uvx pytest tests/test_hk_review_skill.py tests/test_starter_scripts.py`

- [ ] Unit 4: Verify and update lifecycle artifacts
  - Requirements: R50-R51, SC1-SC9
  - Files:
    - Modify `specs/20260501-2250-hk-review-skill-contract/verification.md`
    - Later modify `specs/20260501-2250-hk-review-skill-contract/review.md`
  ...
  - Verification:
    ...
    - `python -m py_compile scripts/harness-kit/_lib/starter.py tests/test_hk_review_skill.py tests/test_starter_scripts.py`
...
- Manual inspection that no `template/.agents/skills/hk-review/scripts/` is required or referenced as required
...
- `hk-review` skill contract remains review-only
...
Plan path: specs/20260501-2250-hk-review-skill-contract/plan.md
