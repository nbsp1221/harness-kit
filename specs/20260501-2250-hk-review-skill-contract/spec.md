---
spec_id: 20260501-2250-hk-review-skill-contract
title: hk-review Skill Contract
status: active
stage: spec
...
# hk-review Skill Contract

## Problem

`harness-kit` now has `hk-spec`, `hk-plan`, and `hk-verify`, but the lifecycle still lacks a repeatable review stage.
...
Without `hk-review`, review tends to collapse into one of three weaker patterns:
...
`hk-review` should close the lifecycle loop by reading the artifacts and current implementation, producing a fresh-context findings and resolution record in `review.md`.

## Why Now

The previous lifecycle items established the first three repo-local skills:

- `hk-spec` starts a spec item and authors only `spec.md`
- `hk-plan` consumes a ready `spec.md` and authors only `plan.md`
- `hk-verify` consumes an implemented item and authors only `verification.md`

The final Phase 1 lifecycle skill is `hk-review`.
It is needed before harness-kit can complete the basic `spec -> plan -> implement -> verify -> review` loop without relying on chat memory.

The `hk-verify` dogfooding pass also showed why review must be its own stage.
...
That is exactly the behavior `hk-review` should standardize.

## Research Basis

Local project decisions already define the intended `hk-review` shape:
...
- The same document says `hk-review` should be a read-only fresh-context quality gate.
...
- `specs/20260501-2112-hk-verify-skill-contract/review.md` is a recent dogfooding example of review fan-out, finding synthesis, resolution tracking, and final verdict.
...
- `R1`: `hk-review` MUST be present in the starter template at `template/.agents/skills/hk-review/`.
- `R2`: `template/.agents/skills/hk-review/SKILL.md` MUST use Agent Skills-compatible frontmatter with `name: hk-review`.
- `R3`: The skill description MUST trigger only when reviewing an implemented and verified harness-kit spec item into `review.md`.
- `R4`: The skill MUST require an explicit spec item path or an unambiguous current spec item from the conversation.
- `R5`: `hk-review` MUST read the selected `spec.md`, `plan.md`, `verification.md`, and existing `review.md` before writing `review.md`.
- `R6`: `hk-review` MUST check that `verification.md` has a Review Handoff before producing a ready verdict.
- `R7`: `hk-review` MUST treat Review Handoff as complete only when it identifies verification status, commands or checks run, pass/fail/blocked/skipped results, manual evidence when applicable, known gaps, residual risks, and recommended next action.
- `R8`: Missing or incomplete Review Handoff content MUST block a ready verdict and be recorded as a review finding.
- `R9`: `hk-review` MUST reconstruct intent from lifecycle artifacts, not hidden implementation-session memory.
- `R10`: `hk-review` MUST inspect the current implementation diff, including untracked files when relevant.
...
- `R12`: `hk-review` MUST NOT run `git add`, `git stash`, `git checkout`, `git clean`, `git reset`, commit, merge, push, or any command that mutates repository or filesystem state.
- `R13`: Untracked files MUST be listed first and read only when relevant to the selected spec item or review scope.
- `R14`: `hk-review` MUST compare implementation and evidence against `spec.md` requirements, success criteria, scope, constraints, assumptions, and open questions.
- `R15`: `hk-review` MUST compare implementation and evidence against `plan.md` implementation units, decisions, expected files, and planned verification.
- `R16`: `hk-review` MUST treat `verification.md` as evidence, not truth.
- `R17`: `hk-review` MUST flag failed, skipped, blocked, missing, weak, or manual-only verification evidence when it affects completion confidence.
- `R18`: `hk-review` SHOULD use delegated fresh-context reviewers when the diff is large, touches security, data, external APIs, filesystem, shell, network, permissions, or changes shared lifecycle behavior; otherwise it MAY perform a single-reviewer pass.
- `R19`: When delegating, `hk-review` SHOULD select only relevant roles from correctness, testing/evidence, maintainability/scope, and security.
...
- `R22`: The parent `hk-review` MUST synthesize delegated or human findings into one deduplicated finding set and remains responsible for the final verdict.
...
- `R27`: New findings produced by `hk-review` MUST default to `open`.
- `R28`: `hk-review` MAY mark a finding `fixed` only when current artifacts or diff provide evidence the issue has been resolved.
- `R29`: `human-accepted-risk` MUST only be used when an explicit human decision is present in current user instructions or reviewed lifecycle artifacts, and `hk-review` MUST record the source of that decision.
...
- `R31`: `hk-review` MAY mark a finding `rejected` when concrete evidence shows the finding is invalid, duplicate, out of scope, or already disproven, and MUST record that rationale.
- `R32`: `hk-review` MUST NOT invent acceptance, deferment, waiver, or risk approval decisions.
...
- `R41`: `hk-review` MAY say "ready for human approval", but MUST NOT claim human approval.
- `R42`: If `review.md` is missing or stub-only, `hk-review` MAY create or replace it.
- `R43`: If `review.md` contains meaningful non-stub content, `hk-review` MUST preserve it and either append a new dated review section or stop and require explicit user instruction to replace or revise the existing review.
- `R44`: `hk-review` MUST NOT silently overwrite prior findings, resolutions, residual risk, or verdicts.
- `R45`: `hk-review` MUST write only the selected spec item's `review.md` in Phase 1.
- `R46`: `hk-review` MUST NOT edit source code, tests, `spec.md`, `plan.md`, or `verification.md`.
- `R47`: `hk-review` MUST NOT fix findings, grant waivers, merge, commit, approve release, or change scope.
- `R48`: Phase 1 `hk-review` MUST NOT require a deterministic script; review requires judgment and synthesis.
...
- `R50`: Tests MUST verify the starter includes the `hk-review` skill and that its skill contract contains artifact reads, Review Handoff completeness, fresh-context review, safe read-only diff inspection, delegated reviewer boundaries, finding schema, severity/resolution/verdict labels, resolution authority, existing review preservation, write-boundary, no-fix, no-approval, and no-script rules.
...
- `SC1`: A user or agent can invoke `hk-review` against a verified spec item and receive a useful `review.md` without remembering the review checklist.
...
- `SC9`: Human-owned risk acceptance is visible as human-owned and cannot be created by `hk-review`.

## Scope

In scope:

- starter-template `hk-review` skill layout
...
- `hk-review` must preserve the existing artifact model: `specs/<YYYYMMDD-HHMM-short-slug>/review.md`.
- `hk-review` must treat `spec.md`, `plan.md`, and `verification.md` as required review inputs.
- `hk-review` must be able to review untracked files because this project may be pre-commit during Phase 1.
- `hk-review` must inspect diffs without mutating git or filesystem state.
- `hk-review` must keep `review.md` readable by humans.
- `hk-review` must be repo-local and installed through the starter template in Phase 1.
...
- `A1`: `template/.agents/skills/hk-review/` is the canonical Phase 1 repo-local installation source.
...
- `A6`: Subagent review fan-out is useful for non-trivial work, but `hk-review` should degrade gracefully to a single reviewer when delegation is unavailable.
...
Spec path: specs/20260501-2250-hk-review-skill-contract/spec.md
...
- Local: `specs/20260501-2112-hk-verify-skill-contract/review.md`
- Local: `specs/20260501-2112-hk-verify-skill-contract/verification.md`
