# hk-verify Skill Contract Verification
...
`hk-verify` implementation has been verified with focused skill/starter tests, full unittest discovery, and Python compilation checks.

The implementation adds the repo-local `hk-verify` skill to the starter template, includes it in starter inventory, and covers the skill contract with dedicated tests.
...
| Starter inventory | pass | `scripts/harness-kit/_lib/starter.py` and `tests/test_starter_scripts.py` include `hk-verify` files; focused pytest passes |
| Skill contract | pass | `tests/test_hk_verify_skill.py` checks frontmatter, explicit spec path, `spec.md`/`plan.md` reads, `Implementation Handoff`, fresh evidence, command derivation/substitution, safety gate, result labels, `verification.md` sections, skipped/blocked/manual details, existing evidence preservation, write boundary, no fixes/review/waivers, and no required script |

## Manual Validation

- `template/.agents/skills/hk-verify/SKILL.md` has `name: hk-verify`.
- The description is narrow: it triggers on converting an implemented harness-kit spec item into `verification.md` evidence.
...
- No `template/.agents/skills/hk-verify/scripts/` directory was added.

## Skipped Checks

- Live Codex runtime invocation of `$hk-verify` was not executed in this verification pass.
  The skill is validated as a starter artifact and contract text, matching the existing `hk-spec` and `hk-plan` test strategy.
