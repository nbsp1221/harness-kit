# hk-verify Skill Contract Plan

## Overview

Implement `hk-verify` as the third repo-local harness-kit lifecycle skill.
...
This plan keeps Phase 1 intentionally light: `hk-verify` is a `SKILL.md` workflow plus minimal agent metadata, not a deterministic verification runner, QA subsystem, or code-fixing tool.

## Requirements Trace

| Spec ID | Requirement | Plan Coverage | Verification |
| --- | --- | --- | --- |
| R1 | Starter contains `template/.agents/skills/hk-verify/` | Unit 1 creates skill directory | File existence tests |
...
- starter inventory updates so bootstrap/adopt install `hk-verify`
- dedicated `hk-verify` skill contract tests
- updating this spec item's verification/review artifacts later in the lifecycle

Out of scope for this pass:

- `template/.agents/skills/hk-verify/scripts/`
...
- `hk-plan` already records expected verification commands and outcomes in `plan.md`.
- `hk-verify` should consume those expectations and record evidence in `verification.md`.
...
| --- | --- | --- | --- |
| Implement `hk-verify` as skill prose plus metadata only | Verification requires command execution and evidence judgment, not scaffolding | Add a command runner script now | R8-R29 |
...
- [ ] Unit 1: Add `hk-verify` skill source
  - Requirements: R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, R16, R17, R18, R19, R20, R21, R22, R23, R24, R25, R26, R27, R28, R29
  - Files:
    - Create `template/.agents/skills/hk-verify/SKILL.md`
    - Create `template/.agents/skills/hk-verify/agents/openai.yaml`
  - Depends on: none
  - Approach:
    - Mirror the concise structure of `hk-plan`.
    - Put `name: hk-verify` and a narrow description in frontmatter.
...
- [ ] Unit 2: Add `hk-verify` to starter inventory
  - Requirements: R1, R30
  - Files:
  ...
  - Depends on: Unit 1
  - Approach:
    - Add `.agents/skills/hk-verify/SKILL.md` and `.agents/skills/hk-verify/agents/openai.yaml` to both required template file lists.
    - Do not add executable-bit assertions because `hk-verify` has no script in Phase 1.
  - Verification:
    - Starter template file test passes.
    - Bootstrap fixture creates the `hk-verify` files.

- [ ] Unit 3: Add dedicated `hk-verify` contract tests
  - Requirements: R2-R30
  - Files:
    - Create `tests/test_hk_verify_skill.py`
  - Depends on: Unit 1
  - Approach:
    - Assert skill files exist.
    - Assert frontmatter has `name: hk-verify`.
    - Assert trigger text mentions implemented harness-kit spec item and `verification.md` evidence.
  ...
  - Verification:
    - `uvx pytest tests/test_hk_verify_skill.py tests/test_starter_scripts.py`

- [ ] Unit 4: Verify and update lifecycle artifacts
  - Requirements: R30, SC1-SC9
  - Files:
    - Modify `specs/20260501-2112-hk-verify-skill-contract/verification.md`
    - Later modify `specs/20260501-2112-hk-verify-skill-contract/review.md`
  ...
  - Verification:
    ...
    - `python -m py_compile scripts/harness-kit/_lib/starter.py tests/test_hk_verify_skill.py tests/test_starter_scripts.py`
...
- `python -m unittest discover -s tests`
- `python -m py_compile scripts/harness-kit/_lib/starter.py tests/test_hk_verify_skill.py tests/test_starter_scripts.py`
- Manual inspection of `template/.agents/skills/hk-verify/SKILL.md`
- Manual inspection that no `template/.agents/skills/hk-verify/scripts/` is required or referenced as required
...
- starter bootstrap/adopt inventory includes `hk-verify`
- `hk-verify` skill contract remains verification-only
...
Plan path: specs/20260501-2112-hk-verify-skill-contract/plan.md
