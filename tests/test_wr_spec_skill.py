...
SKILL_DIR = TEMPLATE_DIR / ".agents" / "skills" / "wr-spec"
SCRIPT = SKILL_DIR / "scripts" / "new-spec-item"


class WrSpecSkillTest(unittest.TestCase):
    ...
    def test_wr_spec_template_files_exist(self):
        self.assertTrue((SKILL_DIR / "SKILL.md").exists())
        self.assertTrue(SCRIPT.exists())
        self.assertTrue((SKILL_DIR / "agents" / "openai.yaml").exists())
        self.assertTrue(SCRIPT.stat().st_mode & 0o111)

    def test_wr_spec_frontmatter_matches_skill_contract(self):
        text = (SKILL_DIR / "SKILL.md").read_text()

        self.assertTrue(text.startswith("---\n"))
        frontmatter = text.split("---", 2)[1]
        self.assertIn("name: wr-spec", frontmatter)
        self.assertIn("starting or creating a new wayrail spec item", frontmatter)
        self.assertIn("Do not use for implementation planning", frontmatter)

    def test_wr_spec_instructions_reference_script_and_templates(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
