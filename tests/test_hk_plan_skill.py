...
SKILL_DIR = TEMPLATE_DIR / ".agents" / "skills" / "hk-plan"


class HkPlanSkillTest(unittest.TestCase):
    def test_hk_plan_template_files_exist(self):
        self.assertTrue((SKILL_DIR / "SKILL.md").exists())
        self.assertTrue((SKILL_DIR / "agents" / "openai.yaml").exists())
        self.assertFalse((SKILL_DIR / "scripts").exists())

    def test_hk_plan_frontmatter_matches_skill_contract(self):
        text = (SKILL_DIR / "SKILL.md").read_text()

        self.assertTrue(text.startswith("---\n"))
        frontmatter = text.split("---", 2)[1]
        self.assertIn("name: hk-plan", frontmatter)
        self.assertIn("ready harness-kit spec.md", frontmatter)
        self.assertIn("writes only plan.md", frontmatter)
        self.assertIn("Do not use for code implementation", frontmatter)

    def test_hk_plan_requires_explicit_or_unambiguous_spec_item(self):
        text = (SKILL_DIR / "SKILL.md").read_text()

        self.assertIn("explicit spec item path", text)
        self.assertIn("unambiguous current spec item", text)

    def test_hk_plan_enforces_readiness_gate(self):
        text = (SKILL_DIR / "SKILL.md").read_text()

        self.assertIn("Planning Handoff", text)
        self.assertIn("Blocked before hk-plan", text)
        self.assertIn("Resolve Before Planning", text)
        self.assertIn("Stop without writing", text)

    def test_hk_plan_requires_bounded_repository_research(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_plan_limits_write_scope_and_lifecycle_boundary(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_plan_defines_plan_authoring_rules(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_plan_classifies_unknowns_before_asking_human(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
