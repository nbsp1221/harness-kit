...
SKILL_DIR = TEMPLATE_DIR / ".agents" / "skills" / "hk-review"


class HkReviewSkillTest(unittest.TestCase):
    def skill_text(self):
        return (SKILL_DIR / "SKILL.md").read_text()

    def test_hk_review_template_files_exist(self):
        self.assertTrue((SKILL_DIR / "SKILL.md").exists())
        self.assertTrue((SKILL_DIR / "agents" / "openai.yaml").exists())
        self.assertFalse((SKILL_DIR / "scripts").exists())

    def test_hk_review_frontmatter_matches_skill_contract(self):
        text = self.skill_text()

        self.assertTrue(text.startswith("---\n"))
        frontmatter = text.split("---", 2)[1]
        self.assertIn("name: hk-review", frontmatter)
        self.assertIn("implemented and verified harness-kit spec item", frontmatter)
        self.assertIn("review.md", frontmatter)
        self.assertIn("Do not use for implementation", frontmatter)

    def test_hk_review_requires_explicit_or_unambiguous_spec_item(self):
        text = self.skill_text()

        self.assertIn("explicit spec item path", text)
        self.assertIn("unambiguous current spec item", text)

    def test_hk_review_reads_required_artifacts_before_writing(self):
        text = self.skill_text()
    ...
    def test_hk_review_enforces_review_handoff_completeness(self):
        text = self.skill_text()
    ...
    def test_hk_review_requires_fresh_artifact_context(self):
        text = self.skill_text()
    ...
    def test_hk_review_defines_safe_read_only_diff_inspection(self):
        text = self.skill_text()
    ...
    def test_hk_review_compares_against_artifacts_and_evidence(self):
        text = self.skill_text()
    ...
    def test_hk_review_defines_delegated_reviewer_boundaries(self):
        text = self.skill_text()
        ...
        self.assertIn("parent `hk-review`", text)
        self.assertIn("final verdict", text)

    def test_hk_review_defines_finding_schema(self):
        text = self.skill_text()
    ...
    def test_hk_review_defines_severity_resolution_and_verdict_labels(self):
        text = self.skill_text()
    ...
    def test_hk_review_defines_review_md_sections_and_no_findings_rule(self):
        text = self.skill_text()
    ...
    def test_hk_review_preserves_existing_review_content(self):
        text = self.skill_text()
    ...
    def test_hk_review_limits_write_scope_and_lifecycle_boundary(self):
        text = self.skill_text()
    ...
    def test_hk_review_does_not_require_phase_one_script(self):
        text = self.skill_text()
