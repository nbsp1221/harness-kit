...
SKILL_DIR = TEMPLATE_DIR / ".agents" / "skills" / "hk-verify"


class HkVerifySkillTest(unittest.TestCase):
    def test_hk_verify_template_files_exist(self):
        self.assertTrue((SKILL_DIR / "SKILL.md").exists())
        self.assertTrue((SKILL_DIR / "agents" / "openai.yaml").exists())
        self.assertFalse((SKILL_DIR / "scripts").exists())

    def test_hk_verify_frontmatter_matches_skill_contract(self):
        text = (SKILL_DIR / "SKILL.md").read_text()

        self.assertTrue(text.startswith("---\n"))
        frontmatter = text.split("---", 2)[1]
        self.assertIn("name: hk-verify", frontmatter)
        self.assertIn("implemented harness-kit spec item", frontmatter)
        self.assertIn("verification.md evidence", frontmatter)
        self.assertIn("Do not use for code implementation", frontmatter)

    def test_hk_verify_requires_explicit_or_unambiguous_spec_item(self):
        text = (SKILL_DIR / "SKILL.md").read_text()

        self.assertIn("explicit spec item path", text)
        self.assertIn("unambiguous current spec item", text)

    def test_hk_verify_reads_spec_and_plan_before_writing(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_enforces_implementation_handoff(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_requires_fresh_planned_check_evidence(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_defines_command_derivation_and_substitution_rules(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_defines_command_safety_gate(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_defines_result_labels(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_defines_verification_md_sections(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_records_skipped_blocked_and_manual_validation_details(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_preserves_existing_verification_evidence(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_limits_write_scope_and_lifecycle_boundary(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
    ...
    def test_hk_verify_does_not_require_phase_one_script(self):
        text = (SKILL_DIR / "SKILL.md").read_text()
