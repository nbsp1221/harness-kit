---
name: wr-review
description: "Use when reviewing an implemented and verified wayrail spec item into review.md. Reads a selected spec item, checks Review Handoff readiness, inspects the current diff read-only, synthesizes findings, resolutions, residual risk, and a verdict, and writes only review.md. Do not use for implementation, verification command execution, fixing findings, waiver approval, merge, commit, release approval, or editing spec.md/plan.md/verification.md."
---

# wr-review

Create a fresh-context review record for an implemented and verified wayrail spec item.

## Use When

- The user asks to run `wr-review`, review a completed wayrail spec item, or move a verified item into review.
...
- The user asks to run verification checks or write verification evidence; use `wr-verify`.
...
The parent `wr-review` synthesizes delegated or human findings into one deduplicated finding set and owns the final verdict.
