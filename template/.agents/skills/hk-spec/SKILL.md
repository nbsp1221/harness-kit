...
Start a new harness-kit spec item.
...
- The user directly invokes `$hk-spec`.
...
This is a repo-local harness-kit skill. Use it only in repositories that contain the harness-kit starter structure, including `specs/_templates/`. If that structure is missing, stop and report that the project has not been initialized for harness-kit instead of inventing files by hand.
