...
function printTopHelp() {
  console.log(`usage: harness-kit <command> [options] [target]

Commands:
  bootstrap    Install the harness-kit starter into a new repository.
  adopt        Inspect and add missing harness-kit starter files to an existing repository.
  doctor       Validate harness-kit starter contract health.

Aliases:
  hks          Short alias for harness-kit.

Run "harness-kit <command> --help" for command options.`);
}
...
function printCommandHelp(command) {
  ...
  console.log(`usage: harness-kit ${command} [--json]${command === "doctor" ? "" : " [--dry-run]"} [target]
...
async function validateConfig(target, checks) {
  const configPath = path.join(target, "harness-kit.yaml");
  if (!await exists(configPath)) {
    addCheck(checks, "config.exists", "config", "fail", "harness-kit.yaml", "harness-kit.yaml is missing");
    return;
  }
  ...
  if (text === null) {
    addCheck(
      ...
      "fail",
      "harness-kit.yaml",
      `harness-kit.yaml could not be read as text: ${error}`,
    );
    return;
  }
  ...
  if (missing.length > 0) {
    addCheck(
      ...
      "fail",
      "harness-kit.yaml",
      "harness-kit.yaml is missing required Phase 1 keys",
    );
  } else {
    addCheck(
      checks,
      ...
      "pass",
      "harness-kit.yaml",
      "harness-kit.yaml has the minimum Phase 1 shape",
    );
  }
}
