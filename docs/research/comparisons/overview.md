...
`harness-kit` is not trying to become the biggest open harness platform.
...
## Current Ranking For Harness Kit
...
- penalty: runtime heaviness that would likely push `harness-kit` past v1 scope
...
It is closest to the actual problem shape: staged work, artifact handoffs, setup as a first-class action, and compounding as a discipline. It is weaker as a runtime benchmark than it first appears, which is actually useful because `harness-kit` v1 also should not start as a full runtime.
...
It is broader than `harness-kit` v1, but it is the best reference in this pass for deterministic workflows, validation, lifecycle operations, and bundled < global < project config precedence.
...
It is the broadest runtime substrate in the target set. That makes it valuable architecturally, but it is less directly aligned with `harness-kit`'s immediate goal than the others.
...
Current best synthesis for `harness-kit`:
...
Run a bounded review pass on the drafted research docs, then freeze the first-pass benchmark conclusions and convert them into a concrete `harness-kit` v1 architecture direction.
